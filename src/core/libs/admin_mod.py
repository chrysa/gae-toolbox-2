...
def list_mod(url):
	...
	:rtype:            liste
	"""
	lst = os.listdir(config.ADMIN_PATH)                                                				        # génération de la liste des modules présents
	print(config.ADMIN_PATH)
	liste = []
	for d in lst:
	    if url == '1':                                                                                      						# vérificaion de l'accès via URL
	        if d not in config.MODULES_BLACKLIST:                                                           						# vérification de l'accessibilité des modules
	            s = config.ADMIN_PATH + os.sep + d                                                        						# génération du chemein relatif du module en cours de traitement
...
def is_mod(module):
	...
	:rtype:			booléen
	"""
	if os.path.isdir(config.ADMIN_PATH + module):
		return True
	else:
		return False
...
def is_controller_page(module, page):
	...
	:rtype:			booléen
	"""
	if os.path.isfile(config.ADMIN_PATH + module + '/controllers/' + page + '.py'):
		return True
	else:
		return False
...
def is_model_page(module, page):
	...
	:rtype:			booléen
	"""
	if os.path.isfile(config.ADMIN_PATH + module + '/models/' + page + '.py'):
		return True
	else:
		return False
...
def load_controller(module, controller):
	mod = __import__(config.MODULES_FOLDER + '.' + config.MODULES_ADMIN_FOLDER + '.' + module + '.controllers.' + controller, fromlist=[controller])
	return mod

def load_model(module, model):
	mod = __import__(config.MODULES_FOLDER + '.' + config.MODULES_ADMIN_FOLDER + '.' + module + '.models.' + model, fromlist=[model])
	return mod
...
class GetModuleCSS(webapp2.RequestHandler):
	...
	def get(self, module, css):
	    ...
	    :rtype:            string
	    """
	    file_path = config.PROJECT_PATH + config.MODULES_ADMIN_FOLDER + os.sep + module + 'medias' + os.sep + 'css' + os.sep + css + '.css'
	    if module != '' or os.path.exists(file_path):
	        return file_path
	    else:
	        return config.PROJECT_PATH + config.MODULES_ADMIN_FOLDER + os.sep + config.ADMIN_HOME + 'medias' + os.sep + 'css' + os.sep + css + '.css'
...
class GetModuleImg(webapp2.RequestHandler):
	...
	def get(self, module, img):
	    ...
	    :rtype:            string
	    """
	    return config.PROJECT_PATH + config.MODULES_ADMIN_FOLDER + os.sep + module + os.sep + 'medias' + os.sep + 'img' + img
...
class GetModuleJS(webapp2.RequestHandler):
	...
	def get(self, module, js):
	    ...
	    :rtype:            string
	    """
	    file_path = config.PROJECT_PATH + config.MODULES_ADMIN_FOLDER + os.sep + module + 'medias' + os.sep + 'js' + os.sep + js + '.j'
	    if module != '' or os.path.exists(file_path):
	        return file_path
	    else:
	        return config.PROJECT_PATH + config.MODULES_ADMIN_FOLDER + os.sep + config.ADMIN_HOME + 'medias' + os.sep + 'js' + os.sep + js + '.js'
	        
class load(webapp2.RequestHandler):
	"""class de gestion des modules pour le chargement a la volée"""
	def get(self, module=config.ADMIN_HOME, page=config.HOME_PAGE, fct=config.HOME_CLASS, url='1'):
	...
	def post(self, module=config.ADMIN_HOME, page=config.HOME_PAGE, fct=config.HOME_CLASS, url='1'):
